# 🔥 Horno de Tezontle – Valorización de Bagazo de Agave

> De residuo a recurso: Biochar, CO₂ y energía limpia


## 📌 Resumen ejecutivo

El presente proyecto desarrolla un **prototipo de horno híbrido con aislamiento de tezontle** para procesar **100 kg/día de bagazo de agave**, produciendo biochar como sumidero de carbono y capturando CO₂ mediante absorción química básica.

El horno, una vez validado, podrá ser escalado para tratar residuos sólidos urbanos orgánicos, **prolongando la vida útil de los rellenos sanitarios** al desviar hasta el 40% de los residuos actualmente depositados.

**Impacto esperado:**
- Reducción de emisiones: **18 t CO₂e/año** a escala piloto
- Captura de CO₂: **>85%** mediante solución de NaOH
- Producción de biochar: **20–25 kg/día** (estabiliza carbono >100 años)
- Generación eléctrica: **0.5–1.0 kWh/día** (motor Stirling)
- Prolongación de rellenos sanitarios: **10–15 años adicionales** al desviar orgánicos

El uso de **tezontle como aislante térmico** en el horno reduce costos de importación de materiales refractarios, aporta identidad tecnológica nacional y permite la construcción de un prototipo de muy bajo costo para la valorización de residuos orgánicos.


## 📊 Contenido de la presentación (8 diapositivas)

| Diapositiva | Contenido |
|-------------|-----------|
| **1** | Portada – Título, autor, institución |
| **2** | El problema – Residuos de agave, emisiones, rellenos sanitarios |
| **3** | La solución – Horno de tezontle, productos (biochar, CO₂, electricidad) |
| **4** | Tecnología clave – Doble pared, control de aire, eficiencia 94% |
| **5** | El equipo – Dr.-Ing. Humberto Peredo Fuentes y colaboradores |
| **6** | Impacto ambiental y social – Emisiones evitadas, captura, ODS |
| **7** | Hoja de ruta – Prototipo → Validación → Escalamiento → Comercialización |
| **8** | Contacto y cierre – Datos de contacto, frase final |


# 📂 Estructura del repositorio
text
Copy
Download
.
├── Deck_Horno_Tezontle_Reducido.pdf   # Presentación compilada
├── README.md                          # Este archivo
└── assets/
    └── tec-logo.png                   # Logo del Tec (opcional, para portada)

# 👨‍🔬 El equipo

| Nombre                           | Rol                               | Institución             |
|----------------------------------|-----------------------------------|-------------------------|
| Dr.-Ing. Humberto Peredo Fuentes |	Investigador Principal         | ITESM Campus Querétaro  |
| CIATEQ A.C.                      |	Desarrollo de prototipos       | Centro de Investigación |
| CIDETEQ	                       | Captura de CO₂	                   | Centro de Investigación |
| TEC Startup Lab	               | Mentoría y networkingInstitución  | Tecnológico de Monterrey|

# 🌱 Impacto esperado
- 18 ton CO₂e/año reducidas por horno
- 3.4 ton CO₂/año capturadas directamente
- Biochar como sumidero de carbono por >100 años
- Prolongación de rellenos sanitarios en 10–15 años
- Empleos verdes: 3–5 operadores por horno

# Soberanía tecnológica: materiales 100% mexicanos

# 📅 Hoja de ruta

| Fase	| Duración	| Hito
| Prototipo|	4 meses	| Construcción del horno piloto
| Validación	| 3 meses	| Pruebas con bagazo de agave
| Certificación| 	2 meses | 	EBC/IBI para biochar
| Escalamiento	| 3 meses	| Optimización y réplica
| Comercialización| 	6 meses | 	Primeros clientes

# 📬 Contacto
Dr.-Ing. Humberto Peredo Fuentes

✉️ humberto.peredo@tec.mx

🔗 LinkedIn

🐙 GitHub

🌐 Sitio web

# 📄 Licencia
Este proyecto está bajo la licencia Creative Commons Attribution-NonCommercial 4.0 International (CC BY-NC 4.0).

Puedes compartir y adaptar el material siempre que:

Des crédito al autor original

No lo uses con fines comerciales

# 🤝 Contribuciones
Las contribuciones son bienvenidas. Si deseas mejorar la presentación o proponer cambios, abre un Issue o envía un Pull Request.

# 📚 Referencias
European Biochar Certificate (EBC)
International Biochar Initiative (IBI)
CONAHCYT – Fondo de startups
Tecnológico de Monterrey – Startup Lab

# “Convertir un problema ambiental en una solución climática, energética y de identidad tecnológica nacional.”

